...
An alternative predictor could be the median income in the zip code of the patients home address *Income_MedicanR* , a `r class(PUF_Q4_2013$Income_MedianR)` with `r n_Income_MedianRNA` NA values. We will generate a separate file with median income as predictor at the bottom. 

There is considerable overlap in the missingness. In other words, data are missing in different subpopulatons for *Income_MedianR* and *Payment*. This makes it feasible to test our hypothesis in this population with complete data on median income at patient address, using *Income_MedianR* as predictor for poor healthcare quality. 
...
We limit the analysis on the complete cases with *Payment* information on insurance status.
...
## Medican Income Dataset

An alternative predictor could be the median income in the zip code of the patients home address *Income_MedicanR* , a `r class(PUF_Q4_2013$Income_MedianR)` with `r n_Income_MedianRNA` NA values. We generate a separate file with median income as predictor, but only two practices are contributing to this data.

```{r}
# Limit to cases with *IncomeMedianR* observed
