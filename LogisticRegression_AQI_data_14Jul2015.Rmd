...
we load the cleaned dataset *myAQI_4_14.Rdata*, which we generated in *import_AQI_14Jul2015.Rmd*   


```{r, echo=TRUE}
rm(list = ls())
load("Analysis/Data/myAQI_4_14.Rdata")
str(myAQI_4_14)
```
...
```{r, echo=TRUE}
formula <- ondansetron ~ Payment +patient_age +patient_sex +
  practiceID + case_duration_minutes 
fit_logistic <- glm(formula, 
                   family = binomial(link = "logit"), data = myAQI_4_14)
```
...
exp(coef(fit_logistic)) # exponentiated coefficients
